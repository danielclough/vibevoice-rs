...
impl Config {
    ...
    fn resolve_voice(&self, voice: &str, model: ModelVariant) -> String {
...
#[derive(Clone)]
struct AppState {
    worker_tx: std_mpsc::Sender<WorkerRequest>,
    config: Arc<Config>,
                ...
                WorkerRequest::SynthesizeStream { text, voice, model, progress_tx } => {
                    ...
                    let result = vv.synthesize_with_callback(
                        ...
                        Some(Box::new(move |progress: Progress| {
                            ...
                            let pcm_bytes = progress.audio_chunk.map(|chunk| {
                            ...
                            let _ = progress_tx_clone.blocking_send(event);
                        })),
                    );
